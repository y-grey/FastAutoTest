...
public class ReportNGUtils {
    ...
    public String getAuthod(ITestResult result) {
        String authod = "--";
        try {
            ...
            if (methods != null && methods.length > 0) {
                for (Method method : methods) {
                    FastAuto authodAnnotation = method.getAnnotation(FastAuto.class);
                    if (authodAnnotation != null &&
                            result.getMethod().getMethodName().equals(method.getName())) {
                        authod = authodAnnotation.author();
                    }
                }
            }
    ...
    public String getRunCounts(ITestResult result) {
        String RunCounts = "1";
        List<String> list = getTestOutput(result);
        for(String s : list){
            if(s.contains("RunCount=")){
                RunCounts = s.replace("RunCount=","");
            }
        }
        return RunCounts;
    }
    ...
    public List<String> getTestOutputWithoutRunCount(ITestResult result) {
